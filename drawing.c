...
void calculate_widget_size(struct BGTK_Context* ctx, struct BGTK_Widget* w) {
	...
	switch (w->type) {
		...
		case BGTK_WIDGET_IMAGE:
			// Image size is its pixel size + padding + margin
			// (outer size)
			break;
...
static void draw_image(struct BGTK_Context* ctx, struct BGTK_Widget w,
	...
	if (!w.data.image.pixels || w.data.image.img_w <= 0 || w.data.image.img_h <= 0) {
		return;
	}
...
void draw_widget(struct BGTK_Context* ctx, struct BGTK_Widget* w,
	...
	switch (w->type) {
		...
		case BGTK_WIDGET_BUTTON: {
			...
			draw_rect(ctx, pixels, w->x + w->margin,
				  w->y + w->margin, w->w - 2 * w->margin, bw,
				  border);  // Top
			draw_rect(ctx, pixels, w->x + w->margin,
				  w->y + w->h - bw - w->margin,
				  w->w - 2 * w->margin, bw,
				  border);  // Bottom
			draw_rect(ctx, pixels, w->x + w->margin,
				  w->y + w->margin, bw, w->h - 2 * w->margin,
				  border);  // Left
			...
			if (w->data.button.label) {
				...
				int lx =
				    content_x0 +
				    (content_w - w->data.button.label->w) / 2 +
				    off;
				int ly =
				    content_y0 +
				    (content_h - w->data.button.label->h) / 2 +
				    off;
		...
		case BGTK_WIDGET_FRAME: {
			...
			// If the whole window is unfocused, dim heavily.
			...
			if (w->data.frame.child) {
				w->data.frame.child->x =
				    w->x + w->margin + bw + w->padding;
				w->data.frame.child->y =
				    w->y + w->margin + bw + w->padding;
		...
		case BGTK_WIDGET_TEXT_INPUT: {
			...
			draw_rect(ctx, pixels, w->x + w->margin,
				  w->y + w->h - bw - w->margin,
				  w->w - 2 * w->margin, bw, border);  // Bottom
			draw_rect(ctx, pixels, w->x + w->margin,
				  w->y + w->margin, bw, w->h - 2 * w->margin,
				  border);  // Left
			draw_rect(ctx, pixels, w->x + w->w - bw - w->margin,
			...
			int pen_x = draw_x;
			int pen_y =
			    text_y + (ctx->ft_face->size->metrics.ascender >> 6);
			...
			for (const char* p = full; *p; p++) {
				...
				for (unsigned int row = 0; row < bitmap->rows; row++) {
					...
					for (unsigned int col = 0; col < bitmap->width;
					     col++) {
						...
						uint8_t r =
						    (r_src * a + r_dst * inv) / 255;
						uint8_t g =
						    (g_src * a + g_dst * inv) / 255;
						uint8_t b =
						    (b_src * a + b_dst * inv) / 255;
